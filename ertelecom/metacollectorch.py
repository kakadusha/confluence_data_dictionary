...
class MetaCollectorCH:
    def __init__(self, db_in_config, connections):
        self.db_in_config = db_in_config
        self.connections = connections
        with open(os.path.join(connections["clickhouse"]["comments_path"], db_in_config["name"] + ".json"), "r") as comments_file:
    ...
    def get_tables_list_from_db(self, database):
        try:
            ...
            result = self.cursor.execute("SHOW TABLES FROM {database}".format(database=database["name"]))
        except Exception as e:
            print(f"[!] Exception on SHOW TABLES FROM {database['name']}:")
            print(type(e))
            print(e.args)
            print(e)
            return []
    ...
    def get_field_description(self, database, table, field, field_comment_id_db):
        if database in self.field_desc:
            db = database
        elif database.lower() in self.field_desc:
            db = database.lower()
        else:
            return "{color:#3e3e0e}" + field_comment_id_db + "{color}"

        if table in self.field_desc[db]["tables"]:
            tb = table
        elif table.lower() in self.field_desc[db]["tables"]:
            tb = table.lower()
        else:
            return "{color:#3e3e0e}" + field_comment_id_db + "{color}"

        if field in self.field_desc[db]["tables"][tb]["fields"]:
            fl = field
        elif field.lower() in self.field_desc[db]["tables"][tb]["fields"]:
            fl = field.lower()
        else:
            return "{color:#3e3e0e}" + field_comment_id_db + "{color}"

        return self.field_desc[db]["tables"][tb]["fields"][fl]
    ...
    def get_table_meta(self, database, table_name):
        ...
        try:
            result = self.cursor.execute("DESCRIBE TABLE {database}.{table}".format(database=database["name"], table=table_name))
            description = result
            ...
            try:
                result = self.cursor.execute(
                    "SELECT * FROM {database}.{table} LIMIT 5 format JSONEachRow".format(database=database["name"], table=table_name)
                )
                sample_data = result
                ...
                for row in sample_data:
                    ...
                    for field_name in field_names:
                        if field_name not in sample:
                            sample[field_name] = []
                        if self.field_desc[database["name"].lower()]["tables"][table_name.lower()].get("sample_hide"):
    ...
    def get_table_statistics(self, database, table_name, table_meta):
        description = self.cursor.execute("SHOW PARTITIONS {database}.{table}".format(database=database["name"], table=table_name))
    ...
    def execute(self):
        # see https://clickhouse-driver.readthedocs.io/en/latest/api.html#connection
        self.cursor = ClickHouse(
            self.connections["clickhouse"]["host"],
            user=self.connections["clickhouse"]["username"],
            password=self.connections["clickhouse"]["password"],
            secure=self.connections["clickhouse"]["secure"],  # secure=True not accepted by server
            verify=False,
            database=self.connections["clickhouse"]["database2connect"],
            compression=self.connections["clickhouse"]["compression"],
        )
        p = self.cursor.execute("SHOW DATABASES")
        meta = self.collect_meta()
        with open(META_FILENAME_PATTERN.format("clickhouse", self.db_in_config["name"]), "w") as meta_file:
